  ...
  (testing "reads CSV with headers"
    ...
    (try
      (let [connector (csv/->CsvConnector)
            ...
            result (csv/pull connector config {})]
        (is (= 2 (count result)))
  ...
  (testing "validates file exists"
    (let [connector (csv/->CsvConnector)]
      (testing "missing file config"
        (let [result (csv/validate connector {})]
          (is (not (:valid? result)))
          (is (= 1 (count (:errors result))))))
      
      (testing "file not found"
        (let [result (csv/validate connector {:file "/nonexistent/file.csv"})]
          (is (not (:valid? result)))
      ...
      (testing "valid file"
        (create-test-csv-file)
        (try
          (let [result (csv/validate connector {:file sample-csv-file})]
            (is (:valid? result))
